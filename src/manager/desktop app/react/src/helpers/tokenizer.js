...
export const tokenize = (user, repository, token, pagename) => {
    const cr = new Cryptography(pagename.toLowerCase(), 0, 0)
    return cr.encrypt(`${user}\n${repository}\n${token}`)
}

export const detokenize = (token, pagename) => {
    const cr = new Cryptography(pagename.toLowerCase(), 0, 0)
    const detokenized = cr.decrypt(token).split("\n")
    return {
        user: detokenized[0],
        repository: detokenized[1],
        accesstoken: detokenized[2]
    }
}
